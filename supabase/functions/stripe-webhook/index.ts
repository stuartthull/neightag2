...
const sendOrderNotification = async (
    ...
    const productName = session.metadata?.product_name || 'TapTag';
    ...
    const text = [
        `A new ${productName} order has been paid.`,
        '',
        `Amount: ${amount}`,
    ...
    await sendEmail({
        ...
        subject: `New ${productName} order - ${amount}`,
        text,
        html: `
      <h1>New ${escapeHtml(productName)} order</h1>
      <p>A ${escapeHtml(productName)} order has been paid and is ready to fulfil.</p>
      <table>
        <tbody>
...
const sendProductConfirmation = async (
    ...
    await sendEmail({
        to: customerEmail,
        subject: `Your NeighTag ${productName} order is confirmed`,
        text: [
            `Hi ${customerName},`,
            '',
            `Thank you for your ${productName} order. Your payment has been received and your order is being prepared.`,
            `Order total: ${amount}`,
        ...
        html: `
      <h1>Your ${escapeHtml(productName)} order is confirmed</h1>
      <p>Hi ${escapeHtml(customerName)},</p>
      <p>Thank you for your ${escapeHtml(productName)} order. Your payment has been received and your order is being prepared.</p>
      ...
      <p>NeighTag</p>
    `,
        idempotencyKey: `${eventId}-product-confirmation`,
    });
};
    ...
    try {
        ...
        if (event.type === 'checkout.session.completed') {
            ...
            if (
                [
                    'taptag',
                    'laminated_stable_tag',
                    'high_gloss_stable_tag',
                    'horsebox_poster',
                ].includes(session.metadata?.order_type || '') &&
                session.payment_status === 'paid'
            ) {
                await sendOrderNotification(session, event.id);
                await sendProductConfirmation(session, event.id);
                console.log(`Sent product order notification for Checkout Session: ${session.id}`);
            } else if (
